...
X_val = [normalize(x[..., 2], 1, 99.8, axis=axis_norm) for x in tqdm(X)]
...
model = StarDist2D(None, name="stardist_multiclass", basedir="training_1_channel_stardist_classifier/models")
...
for class_id in range(1, 4):
    for class_id_pred in range(1, 4):
        ...
        if class_id == class_id_pred:
            ...
            ax2.grid()
            ax2.legend()
            plt.savefig(f"metrics_1ch_class_{class_id}.png")
            plt.close()
